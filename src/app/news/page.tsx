...
import { Meta, News, handleFetchNewsBySearch } from '@/utils/newsUtilities';
...
export default function News() {
    ...
    const [newsDatas, setNewsDatas] = useState<News[] | undefined>([]);
    ...
    useEffect(() => {
        const fetchNewsList = async () => {
            const response = await handleFetchNewsBySearch(searchTerm, currentPage, limitPage, order, selectedCategory, selectedStatus);

            if (response.status === 200 && response.datas && response.meta) {
                setMetaData(response.meta);
                setNewsDatas(response.datas);
        ...
        <>
            ...
            <div className="w-full px-2 md:px-24 py-5 flex flex-col items-center gap-5 text-text dark:text-darkText">
                <div className='w-full flex items-center justify-between'>
                    <p className='text-2xl/7 font-bold sm:truncate sm:text-3xl sm:tracking-tight mr-auto'>News</p>
                    {hasPermission('news-create') && <a className='inline-flex items-center rounded-md bg-primary px-3 py-2 text-sm font-semibold text-white shadow-xs hover:bg-primary/80 button-ring' href='/news/new'>Add News</a>}
                </div>
                        ...
                        <tbody className='w-full'>
                            {
                                metaData.totalItems ?
                                    ...
                                    :
                                    <tr>
                                        <td colSpan={4} className='text-center py-4 text-secondaryText dark:text-darkSecondaryText'>No news available.</td>
                                    </tr>
                            }
                        </tbody>
                    </table>
