...
import { DeleteCategory } from '@/utils/categoryUtilities';
...
export default function CategoryCard({ category, reFetchData }: CategoryCardProps) {
    ...
    const handleDeleteCategory = async (categoryId: number) => {
        setIsOpen(!isOpen);
        DeleteCategory(categoryId, reFetchData);
    }
    ...
    return (
        ...
        >
            ...
            <div className="flex-grow">
                <p className="text-lg font-semibold text-gray-800 truncate">{category.name}</p>
                        ...
                        <div className="relative p-4 w-full max-w-2xl max-h-full">
                            <div className="relative bg-white rounded-lg shadow">
                                <div className="flex items-center justify-between p-4 md:p-5 border-b rounded-t">
                                    <h3 className="text-xl font-semibold text-gray-900">Are you sure you want to delete this category?</h3>
                                ...
                                <div className="flex items-center justify-end p-4 md:p-5 rounded-b gap-2.5">
                                    ...
                                    <button onClick={() => handleDeleteCategory(category.id)} className="text-white bg-red-600 hover:bg-red-700 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">Yes</button>
                                </div>
                            </div>
                        </div>
                    </div>
